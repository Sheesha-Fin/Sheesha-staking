...
import { ERC20Mock, ERC20D6Mock, JustStaking } from "../../typechain";
...
describe("JustStaking integration test for 6 decimals", function () {
  this.timeout(0);
  ...
  beforeEach(
    "Setting up for JustStaking function test for 6 decimals reward token",
    async function () {
      ...
      const fERC20D6MockFactory =
        await ethers.getContractFactory("ERC20D6Mock");

      usdt = await fERC20MockFactory.deploy("USDT");
      usdc = await fERC20D6MockFactory.deploy("USDC");
      ...
      const fJustStakingFactory =
        await ethers.getContractFactory("JustStaking");
